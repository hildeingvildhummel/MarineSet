"""https://github.com/facebookresearch/ssl-data-curation/blob/main/src/hierarchical_sampling.py"""
...
def find_subcluster_target_size(
    ...
    # print(' subcluster sizes: ', subcluster_sizes, arr, multiplier, target_size)
    ...
    if best_cut_left == np.max(arr):
        ...
        remainder = int(target_size - subcluster_target_sizes.sum())
        if remainder > 0:
...
def recursive_hierarchical_sampling(
    ...
    if level == 1:
        ...
        if len(subcl_indices) == 0:
            return np.array([], dtype=np.int64)
        # print('sub cl indices: ', subcl_indices, cl_index, clusters.clusters)
        # print('level -1: ', level-1, subcl_indices, clusters.flat_clusters_size)
...
def hierarchical_sampling(
    ...
    # print(clusters.flat_clusters_size[n_levels], target_size, multiplier)
    ...
    for cl_index in tqdm(
        ...
        if cluster_target_sizes[cl_index] == 0:
            continue

        # print('Sizes: ', clusters.clusters.size, clusters.clusters[cl_index].size)

        # print('Clusters: ', clusters, n_levels, cluster_target_sizes[cl_index], cl_index, multiplier, sampling_strategy)
        samples.append(
            recursive_hierarchical_sampling(
                clusters,
                n_levels,
